...
{
    IAsyncEnumerable<ChatResponse> CompleteChatStreamingAsync(IEnumerable<ChatMessage> messages);
}
...
public class ChatClient(HttpClient http) : IChatClient
{
    ...
    public async IAsyncEnumerable<ChatResponse> CompleteChatStreamingAsync(IEnumerable<ChatMessage> messages)
    {
